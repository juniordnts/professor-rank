...
const fs = require("fs");
const jsonRaw = require("./public/dados/2019-2017.json");
...
jsonRaw.forEach((element) => {
  ...
  if (jsonCluster[element["Docente"]]["Componentes"][element["Componente Curricular"]]) {
    jsonCluster[element["Docente"]]["Componentes"][element["Componente Curricular"]]["Notas"].push(
      parseFloat(element["Média Geral"])
    );
    jsonCluster[element["Docente"]]["Componentes"][element["Componente Curricular"]][
      "Desvios"
    ].push(parseFloat(element["Desvio Padrão Geral"]));
  } else {
    jsonCluster[element["Docente"]]["Componentes"][element["Componente Curricular"]] = {
      Notas: [],
      Desvios: [],
    };
    jsonCluster[element["Docente"]]["Componentes"][element["Componente Curricular"]]["Notas"] = [
      parseFloat(element["Média Geral"]),
    ];
    jsonCluster[element["Docente"]]["Componentes"][element["Componente Curricular"]]["Desvios"] = [
      parseFloat(element["Desvio Padrão Geral"]),
    ];
  }

  if (jsonClusterClasses[element["Componente Curricular"]]) {
    jsonClusterClasses[element["Componente Curricular"]]["Notas"].push(
      parseFloat(element["Média Geral"])
    );
    jsonClusterClasses[element["Componente Curricular"]]["Desvios"].push(
      parseFloat(element["Desvio Padrão Geral"])
    );

    if (
      !jsonClusterClasses[element["Componente Curricular"]]["Docentes"].includes(element["Docente"])
    ) {
    ...
    jsonClusterClasses[element["Componente Curricular"]]["Notas"] = [
      parseFloat(element["Média Geral"]),
    ];
    jsonClusterClasses[element["Componente Curricular"]]["Desvios"] = [
      parseFloat(element["Desvio Padrão Geral"]),
    ];
...
for (k in jsonCluster) {
  ...
  let mediaDeNotas =
    jsonCluster[k]["Notas"].reduce((total, number) => {
      return total + number;
    }, 0) / jsonCluster[k]["Notas"].length;
  let mediaDeDesvios =
    jsonCluster[k]["Desvios"].reduce((total, number) => {
      return total + number;
    }, 0) / jsonCluster[k]["Desvios"].length;
  ...
  for (ki in jsonCluster[k]["Componentes"]) {
    ...
    let mediaDeNotasComponente =
      jsonCluster[k]["Componentes"][ki]["Notas"].reduce((total, number) => {
        return total + number;
      }, 0) / jsonCluster[k]["Componentes"][ki]["Notas"].length;
    let mediaDeDesviosComponente =
      jsonCluster[k]["Componentes"][ki]["Desvios"].reduce((total, number) => {
        return total + number;
      }, 0) / jsonCluster[k]["Componentes"][ki]["Desvios"].length;
    ...
    var notaMedianaComponente =
      notasOrdenadasComponente[
        Math.round((jsonCluster[k]["Componentes"][ki]["Notas"].length - 1) / 2)
      ];
...
for (k in jsonClusterClasses) {
  ...
  let mediaDeNotas =
    jsonClusterClasses[k]["Notas"].reduce((total, number) => {
      return total + number;
    }, 0) / jsonClusterClasses[k]["Notas"].length;
  let mediaDeDesvios =
    jsonClusterClasses[k]["Desvios"].reduce((total, number) => {
      return total + number;
    }, 0) / jsonClusterClasses[k]["Desvios"].length;
...
async function refineNotasAndDesvios(dataCluster) {
  ...
  fs.writeFile("./public/dados/2019-2017-refined.json", dataClusterString, function (err) {
    if (err) throw err;
  ...
  fs.writeFile(
    "./public/dados/2019-2017-refined-classes.json",
    dataClusterClassesString,
    function (err) {
      if (err) throw err;
  ...
  fs.writeFile("./public/dados/usefullData.json", usefullData, function (err) {
    if (err) throw err;
